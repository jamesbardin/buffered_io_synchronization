...
io61_file* io61_fdopen(int fd, int mode) {
    ...
    off_t fsize = io61_filesize(f);
    off_t sections = (fsize + 255)/256;
    f->rlock = new region_lock[sections];
    return f;
}
...
int io61_try_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    std::unique_lock guard(f->rm1);
    
    int rstart = (start / 256), rend = ((start + len - 1) / 256);
    bool b = false;
    for (int ri = rstart; ri <= rend; ++ri) {
        if (f->rlock[ri].locked > 0 && f->rlock[ri].owner != std::this_thread::get_id()) {
            b = true;
            break;
        }
    }
    ...
    for (int ri = (start / 256); ri <= ((start + len - 1) / 256); ++ri) {
        ++f->rlock[ri].locked;
        f->rlock[ri].owner = std::this_thread::get_id();
    }
    return 0;
}
...
bool possible_overlap(io61_file* f, off_t start, off_t len) {
    int rstart = (start / 256), rend = ((start + len - 1) / 256);
    for (int ri = rstart; ri <= rend; ++ri) {
        if (f->rlock[ri].locked > 0 && f->rlock[ri].owner != std::this_thread::get_id()) {
            return true;
...
int io61_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    while (possible_overlap(f, start, len)) {
        f->cvar.wait(guard);
    }
    for (int ri = (start / 256); ri <= ((start + len - 1) / 256); ++ri) {
        ++f->rlock[ri].locked;
